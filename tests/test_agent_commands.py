...
def test_controller_state_uses_knowledge_controller_output_for_observation_preview(tmp_path) -> None:
    config = Config(
        provider=ProviderConfig(),
        telegram=TelegramConfig(),
        daemon=DaemonConfig(
    ...
    db = Database(config.daemon.db_path)
    agent = AssistantAgent(config, db, DummyLLM())
    knowledge = agent.tools.get("knowledge")
    runtime = RuntimeState(request="analyze local files")
    result = ToolResult(
        ok=True,
        summary="Analyzed 1 readable file(s).",
        data={
            "grounded": True,
            "partial": False,
            "scanned_files": 1,
            "scan_truncated": False,
            "supported_files": [{"path": "/repo/notes.txt", "file_type": "txt"}],
            "unsupported_files": [],
            "chunks": [{"chunk_id": "notes.txt:1", "path": "/repo/notes.txt", "text": "hello"}],
            "artifacts": {},
        },
    )
    runtime.append(
    ...
    latest_preview = controller_state["latest_observation"]["data_preview"]
    assert latest_preview["grounded"] is True
    assert latest_preview["supported_files"] == [{"path": "/repo/notes.txt", "file_type": "txt"}]
    assert "chunks" not in latest_preview
    db.close()
